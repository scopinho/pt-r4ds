...
Para finalizar a seção de programação, faremos um rápido tour pelas funções do R base (*base R*) mais importantes que não discutiremos no livro.
Essas ferramentas são particularmente úteis à medida que você programa mais e o ajudarão a ler código R que encontrará por aí.
...
Não é possível usar o tidyverse sem usar o R base, então na verdade já ensinamos a você **muitas** funções do R base: de `library()` para carregar pacotes, até `sum()` e `mean ()` para sumarizações numéricas, para os tipos de dados fatores, data e POSIXct e, claro, todos os operadores básicos como `+`, `-`, `/`, `*`, `|`, `&`, e `!`.
...
Sem dúvida, você encontrará essas outras abordagens quando começar a ler o código R escrito por outras pessoas, principalmente se estiver usando o site *StackOverflow*.
É 100% correto escrever código que use uma combinação de abordagens e não deixe ninguém lhe dizer o contrário!

Neste capítulo, vamos nos concentrar em quatro grandes tópicos: subconjuntos (*subsetting*) com `[`, subconjuntos com `[[` e `$`, a família de funções apply e loops `for`.
Para finalizar, discutiremos brevemente duas funções essenciais de plotagem.

### Pré-requisitos

Este capítulo se concentra no R base (*base R*), portanto não possui nenhum pré-requisito real, mas carregaremos o tidyverse para explicar algumas das diferenças e o pacote dados para o conjunto de dados diamante.
...
`[` é usado para extrair subcomponentes de vetores e *data frames* e é chamado como `x[i]` ou `x[i, j]`.
...
1.  **Um vetor de inteiros positivos**.
    Extrair um subconjunto com inteiros positivos obtem os elementos dessas posições:
...
3.  **Um vetor lógico**.
    ...
    Isto é mais frequentemente útil em conjunto com a função de comparação.

    ```{r}
    x <- c(10, 3, NA, 5, 8, 1, NA)

    # Todos os valores não faltantes (*non-missing values*) de x
    x[!is.na(x)]
...
4.  **Um vetor de caracteres**.
    Se você tiver um vetor nomeado (*named vector*), poderá extrair um subconjunto com um vetor de caracteres:
...
Existem algumas maneiras diferentes[^base-r-1] de usar `[` com um quadro de dados, mas a maneira mais importante é selecionar linhas e colunas independentemente com `df[linhas, colunas]`. Aqui `linhas` e `colunas` são vetores conforme descrito acima.
...
# Selecione todas as linhas e colunas x e y
df[, c("x" , "y")]

# Selecione linhas onde `x` é maior que 1 e todas as colunas
df[df$x > 1, ]
```
...
Precisamos usá-lo aqui porque `[` não usa avaliação organizada (*tidy evaluation*), então você precisa ser explícito sobre a origem da variável `x`.

Há uma diferença importante entre tibbles e *data frames* quando se trata de `[`.
Neste livro, usamos principalmente tibbles, que *são* quadros de dados, mas eles ajustam alguns comportamentos para tornar sua vida um pouco mais fácil.
...
Uma maneira de evitar essa ambigüidade com `data.frame`s é especificar explicitamente `drop = FALSE`:
...
-   `filter()` é equivalente a subconjunto das linhas com um vetor lógico, tomando cuidado para excluir valores ausentes:
    ...
    Outra técnica comum é usar `what()` por seu efeito colateral de eliminar valores ausentes: `df[what(df$x > 1), ]`.

-   `arrange()` é equivalente a subconjunto das linhas com um vetor inteiro, geralmente criado com `order()`:
...
-   Ambos `select()` e `relocate()` são semelhantes a subconjuntos das colunas com um vetor de caracteres:
...
O R base também fornece uma função que combina os recursos de `filter()` e `select()`[^base-r-2] chamada `subset()`:
...
`[`, que seleciona muitos elementos, é emparelhado com `[[` e `$`, que extraem um único elemento.
Nesta seção, mostraremos como usar `[[` e `$` para extrair colunas de *data frames*, discutiremos mais algumas diferenças entre `data.frames` e tibbles e enfatizaremos algumas diferenças importantes entre `[ ` e `[[` quando usado com listass.
...
Eles também podem ser usados ​​para criar novas colunas, no R base é o equivalente a `mutate()`:
...
Existem várias outras abordagens de base R para criar novas colunas, incluindo `transform()`, `with()` e `within()`.
...
`pull()` pegam um nome de variável ou uma posição de variável e retornam apenas aquela coluna.
...
Existem algumas diferenças importantes entre tibbles e `data.frame`s básicos quando se trata de `$`.
Os *data frames** correspondem ao prefixo de qualquer nome de variável (chamado de **correspondência parcial** (*partial matching*) ) e não reclamam se uma coluna não existir:
...
Tibbles são mais rigorosos: eles apenas correspondem exatamente aos nomes das variáveis ​​e irão gerar um aviso se a coluna que você está tentando acessar não existir:
...
A diferença entre `[` e `[[` é particularmente importante para listas porque `[[` detalha a lista enquanto `[` retorna uma lista nova e menor.
Para ajudá-lo a lembrar a diferença, dê uma olhada no pimenteiro incomum mostrado na @fig-pepper.
...
#|   No meio está a foto de um único pacote de pimenta dentro do pimenteiro. À direita esta a 
...
No @sec-iteration, você aprendeu técnicas tidyverse para iteração como `dplyr::across()` e a família de funções map.
...
Infelizmente `tapply()` retorna seus resultados em um vetor nomeado (*named vector*) que requer alguma ginástica se você quiser coletar múltiplos resumos e agrupar variáveis ​​em um *data frame* (é certamente possível não fazer isso e apenas trabalhar com vetores flutuantes livres (*free floating vectors*), mas em nossa experiência isso apenas atrasa o trabalho).
...
Isso raramente aparece na ciência de dados porque geralmente trabalhamos com quadros de dados e não com matrizes.
...
Muitos usuários de R que não usam o tidyverse preferem ggplot2 para plotagem devido a recursos úteis como padrões sensatos, legendas automáticas e uma aparência moderna.
No entanto, as funções de plotagem do R base ainda podem ser úteis porque são muito concisas – é preciso muito pouca digitação para fazer um gráfico exploratório básico.
...
# Esquerda
hist(diamente$quilate)

# Direita
plot(diamonds$quilate, diamonds$preco)
```

Observe que as funções de plotagem básica funcionam com vetores, então você precisa extrair colunas do *data frame* usando `$` ou alguma outra técnica.

## Resumo

Neste capítulo, mostramos uma seleção de funções do R base úteis para subconjuntos (*subsetting*)e iterações.
Em comparação com as abordagens discutidas em outras partes do livro, essas funções tendem a ter mais um sabor de "vetor" do que de "data frames", porque as funções do R base (*base R)* tendem a usar vetores individuais, em vez de um *data frame* e alguma especificação de coluna.
...
Esperamos que esses capítulos tenham despertado seu interesse em programação e que você esteja ansioso para aprender mais fora deste livro.
