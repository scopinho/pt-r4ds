...
Até agora você viu Quarto sendo usado para produzir documentos HTML.
Este capítulo dá uma visão geral de alguns dos muitos outros tipos de saída que você pode produzir com Quarto.
...
2.  Transientemente, chamando `quarto::quarto_render()` manualmente:
...
Quarto oferece uma ampla variedade de formatos de saída.
É possível achar a lista completa em <https://quarto.org/docs/output-formats/all-formats.html>.
Muitos formatos compartilham configurações de seu resultado (por exemplo, `toc: true` para incluir um índice), mas outros têm configurações que são específicas do formato (por exemplo, `code-fold: true` colapsa os blocos de código em um tag HTML `<details>` de modo que o usuário possa exibí-lo sob demanda, não é aplicável a um documento PDF ou Word).

Para sobrescrever as opções padrão, é necessário usar um campo `format` extendido.
Por exemplo, se quiser renderizar um `html` com um índice flutuante, é necessário usar:
...
Note a notação especial (`pdf: default`) caso não queira sobrescrever nenhuma das opções padrão.
...
Há bastantes variações básicas nesse tema, gerando diferentes tipos de documentos.
Por exemplo:
...
Em apresentações o conteúdo do trabalho é dividido em slides, com um novo slide começando em cada marcador de título de nível dois (`##`).
Além disso, títulos de primeiro nível (`#`) indicam o começo de uma nova seção com um slide de título de seção que, por padrão, fica centralizado no meio do slide.
...
-   [**DiagrammeR**](https://rich-iannone.github.io/DiagrammeR) para diagramas (como diagramas de fluxo ou diagramas simples de nodo e aresta -- grafos).
...
Por outro lado, isso é ótimo porque é possível distribuir o arquivo HTML sem qualquer conexão com R.
...
Quando se executa aplicativos Shiny em seu próprio computador, um servidor Shiny é automaticamente configurado para você, mas é nevessário um servidor de acesso público se quiser publicar esse tipo de interatividade online.
...
Para aprender mais sobre Shiny, recomendamos a leitura de Mastering Shiny por Hadley Wickham, [https://mastering-shiny.org](https://mastering-shiny.org/).
...
Por exemplo, o arquivo `_quarto.yml` a seguir cria um site a partir de três arquivos: `index.qmd` (a home page), `viridis-colors.qmd`, e `terrain-colors.qmd`.
...
Recomendamos que se use um Rstudio project para seus sites e livros.
...
-   Você pode gerar transportar documentos Quarto para Jupyter Notebooks com `format: ipynb`: <https://quarto.org/docs/reference/formats/ipynb.html>.

Veja <https://quarto.org/docs/output-formats/all-formats.html> para uma lista de ainda mais formatos.
