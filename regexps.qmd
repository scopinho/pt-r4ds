...
Este capítulo se concentrará em funções que usam **expressões regulares**, uma linguagem concisa e poderosa para descrever padrões (*patterns*) dentro de cadeia de caracteres (*strings*).
O termo "expressão regular" é um pouco longo, então a maioria das pessoas o abrevia como "regex"[^regexps-1] ou "regexp".
...
Podemos também usar a`str_detect()` com `summarize()` combinando-a com `sum()` ou `mean()`: `sum(str_detect(x, padrao))` informa o numero de observações correspondentes ao padrão e `mean(str_detect(x, padrao))` informa a proporção da correspondência.
...
A expressão regular diz, duas e não três vezes:
