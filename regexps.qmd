...
Este capítulo se concentrará em funções que usam **expressões regulares**, uma linguagem concisa e poderosa para descrever padrões (*patterns*) em cadeia de caracteres (*strings*).
...
Se você se encontrar bloqueado com uma abordagem, em geral, pode ser melhor mudar abordagem e atacar seu problema sob uma perspectiva diferente.
...
`str_remove()` e `str_remove_all()` são funções atalhos para `str_replace(x, padrao, "")`:
...
1.  Como você faria para identificar a *string* literal `"'\`? E a `"$^$"`?
