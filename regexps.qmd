...
Isto acontece porque as expressões regulares consideram a diferença entre letras maiúsculas e minúsculas (*case sensitive*)\[\^regexps-nt-1\].
\[\^regexps-nt-1\]: **NT**: Dizemos que a linguagem de programação é *case sensitive* quando existe a diferença entre letras maiúsculas (*upper case*) e minúsculas (*lower case*).
...
Nós usamos strings para representar expressões regulares, e `\` é também usada como símbolo de escapadas nas *strings*. Portanto, para criar a expressão regular `\.` , precismos da *string* `"\\."`, como mostra os exemplos a seguir.
