...
voos <- voos |> mutate(
  horario_saida = hour * 60 + minute,
  arr_time = (arr_time %/% 100) * 60 + (arr_time %% 100),
  airtime2 = arr_time - horario_saida,
  dep_sched = horario_saida + atraso_saida
)

ggplot(voos, aes(x = dep_sched)) + geom_histogram(binwidth = 60)
ggplot(voos, aes(x = dep_sched %% 60)) + geom_histogram(binwidth = 1)
ggplot(voos, aes(x = tempo_voo - airtime2)) + geom_histogram()
```

1.  Compare `horario_saida`, `sched_horario_saida`, e `atraso_saida`.
    Como você esperaria que esses três números estivessem relacionados?

2.  Esboce a maior quantidade possível de maneiras de selecionar `horario_saida`, `atraso_saida`, `arr_time`, e `atraso_chegada` a partir de `voos`.
