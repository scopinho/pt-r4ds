...
Em ambas as visualizações, o nome das variáveis estão seguidos por abreviaturas que te indicam o tipo de cada variável: `<int>` para inteiros, `<dbl>` double[^data-transform-1] (números reais), `<chr>` para caracteres (strings) e `<dttm>` para date-time (data e tempo).
...
Mesmo que esse pipeline tenha quatro etapas, é fácil passar o olho e entendê-lo pois os verbos vêm no início de cada linha: começa com os dados de `flights`, então filtra (*filter*), então modifica (*mutate*), então seleciona (*select*), então ordena(*arrange*).
...
`group_by()` não modifica seus dados mas, se você observar a saída de perto, você verá que a há uma indicação de que ela está agrupada (*"grouped by"*) mês (`Groups: month [12]`).
...
Isso ocorreu pois alguns dos vôos observados possuíam dados ausentes na coluna de atrasos (*delay*), e por isso, quando calculamos a média incluindo esses valores, obtemos um `NA` como resultado.
...
1.  Qual companhia aérea (*carrier*) possui a pior média de atrasos?
...
Especificamente, iremos comparar qual a proporção de vezes um jogador acertou a bola (*"hit"*) (`H`) vs. o número de vezes que ele tentou colocar a bola em jogo (`AB`):
