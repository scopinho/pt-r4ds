...
Nessa seção, focaremos nas funções mais importantes: `group_by()`, `summarize()`, e a família de funções _slice_. 
...
1.  A variação em `performance` é maior entre os jogadores que foram menos vezes ao bastão. 
    A forma do gráfico é muito característica: sempre que você projetar no gráfico uma média (ou alguma outra sumarização estatística)  vs. tamanho do grupo, você verá que a variação diminui à medida que o tamanho da amostra aumenta[^data-transform-4].
