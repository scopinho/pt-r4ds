...
A operação agrupada mais importante é a sumarização (_"summary"_), que caso esteja sendo utilizada para calcular uma única sumarização estatística, reduz o data frame para ter apenas uma única linha para cada grupo.
...
1.  A variação em `performance` é maior entre os jogadores que foram menos vezes ao bastão. 
    A forma do gráfico é muito característica: sempre que você plotar uma média (ou alguma outra sumarização estatística)  vs. tamanho do grupo, você verá que a variação diminui à medida que o tamanho da amostra aumenta[^data-transform-4].
