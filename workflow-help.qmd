...
Então execute 'reprex()', aonde a saída padrão está formatada para o Github:
...
Existem três coisass que você precisa incluir para tornar seu exemplo reproduzível: pacotes necessários, dados e código.

1. **Packages** devem ser carregados no início do script para facilitar a visualização de quais são necessários.
...
2. A maneira mais fácil para incluir os dados é usar 'dput()' para gerar o código em R necessário para recriar os dados.
   Por exemplo, para recriar o dataset 'mtcars' no R rode os seguintes comandos:

   1. Rode 'dput(mtcars)' no R
   2. Copie a saída
   3. No reprex, digite 'mtcars <-' e então cole.

   Tente usar a menor parte dos seus dados que ainda contém o erro.
...
3. Dedique um pouco de tempo para se certificar de que seu código é de fácil leitura por todos:
   ...
   - Use comentários para indicar aonde está o problema;
...
A criação de reprexes não é trivial e exige alguma prática para aprender a criar bons e de fato mínimos reprexes.
Contudo, aprender a fazer perguntas que incluem o código e dedicando algum tempo para fazer ele reproduzível será um aprendizado de longo prazo enquanto você aprender a dominar o R.
...
Para se manter conenctado de maneira geral com a comunidade do R, recomendamos a leitura do R weekly (https://rweekly.org): é um esforça da comunidade em agregar as notícias mais interessantes sobre o R a cada semana.

## Sumário

Este capítulo encerra a parte  Whole Game deste livro.
Você viu até agora as partes mais importantes do processo em ciência de dados: visualização, transformação, classificação e importação.
Agora você tem uma visão abrangente do processo como um todo e começamos a entrar nos detalhes pormenorizados.

A próxima parte do livro, Visualização, é um aprofundamento na estrutura dos gráficos e criação de visualização de dados com o ggplot2, exemplos de como usar as ferramentas que aprendeu até agora para construir uma análise exploratória de dados e também introduzir boas práticas na criação de desenhos para comunicação.
