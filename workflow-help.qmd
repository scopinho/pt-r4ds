...
Este livro não é uma ilha; não existe nenhuma fonte específica que fará você se tornar um expert em R.
Assim que você começar a aplicar aos seus dados as técnicas descritas neste livro, você logo terá questionamentos que nós não respondemos.
Esta seção elenca algumas dicas em como obter ajuda e também lhe ajudar a continuar aprendendo.
...
Além do mais, adicionando na busca nomes de pacotes como "tidyverse" ou "ggplot2" irá ajudar a direcionar os resultados ao seu código de maneira mais familiar a você, e.g., "como fazer um boxplot no R com ggplot2" ao invés de "como fazer um boxplot no R".
Particularmente, Google é muito bom com mensagens de erro.
Se você tiver uma mensagem de erro e não tem ideia do que ela significa, dê um Google com a mensagem!
Há chances de que outros também tenham ficado confusos com o erro anteriormente e em algum lugar da web você terá ajuda.
(Se a mensagem de erro não estiver em Inglês, rode `Sys.setenv(LANGUAGE = "en")` e rode novamente o código; provavelmente encontrará ajuda com mensagens de erro em inglês.)

Se o Google não ajudar tente Stack Overflow (https//stackoverflow.com).
Comece reservando tempo para buscar uma resposta já existente, incluindo '\[R\]' para restringir sua busca por perguntas e respostas que contenham R

## Executando um reprex

ISe a sua busca no Google não retornar nenhuma resposta satisfatória, é uma boa ideia preparar um **reprex,** termo para **repr**oducible **ex**ample.
Um bom reprex facilita para que outras pessoas possam lhe ajudar, e geralmente você poderá resolver o problema durante a criação dele.
Existem duas partes na criação do reprex:

-   Primeiro, você precisa permitir a reprodução do seu código.
    Isto é, significa que você precisa copiar tudo, i.e. incluindo qualquer comando `library()` e criar todas os objetos necessários.
...
-   Segundo, você precisa ser minimalista.
    ...
    Usualmente isto envolve a criação de um objeto R menor e mais simples do que aquele que você está criando de verdade ou até mesmo usar dados internos.

Isto parece muito trabalhoso!
E pode ser, mas tem um grande retorno:

-   Em 80% dos casos criando um excelente reprex pode revelar a raiz do seu problema.
    É incrível como o processo de escrever um exemplo pequeno e auto-sufciente ajuda você a responder sua própria dúvida..
