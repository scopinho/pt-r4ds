...
Na maioria das outras linguagens, você precisaria duplicar explicitamente cada elemento de `x` usando algum tipo de loop *for*.
...
-   `group_by()` junto com a `summarize()` calcula estatísticas sumarizadas para cada subconjunto.
-   `unnest_wider()` e `unnest_longer()` criam novas linhas e colunas para cada elemento de uma coluna de lista (*list-column*).
...
Aprender programação funcional pode facilmente se tornar abstrato, mas neste capítulo manteremos as coisas concretas, concentrando-nos em três tarefas comuns: modificar múltiplas colunas, ler vários arquivos e salvar vários objetos..

### Pré-requisitos

Neste capítulo, focaremos nas ferramentas fornecidas peloas pacotes dplyr e purrr, ambos membros principais do tidyverse.
Você já viu o dplyr antes, mas o [purrr](http://purrr.tidyverse.org/) é novo.
Usaremos apenas algumas funções do purrr neste capítulo, mas é um ótimo pacote para explorar à medida que você aprimora suas habilidades de programação. Usaremos também o pacote `dados` e seu conjunto de dados `diamante`.
...
Imagine que você tem esse problema simples e deseja contar o número de observações e calcular a mediana de cada coluna.
...
`everything()` é simples: seleciona todas as colunas (não agrupadas):
...
`median()` propaga esses valores ausentes, nos dando uma saída abaixo do ideal:
...
Isso é um pouco detalhado, então R vem com um atalho útil: para esse tipo de função descartável, ou **anônima**[^iteration-1], você pode substituir `function` por `\`[^iteration-2 ]:

[^iteration-1]: Anônimo, porque nunca lhe demos explicitamente um nome com `<-`.
...
Se você olhar com atenção, poderá intuir que as colunas são nomeadas usando uma especificação de cola (@sec-glue) como `{.col}_{.fn}` onde `.col` é o nome da coluna original e `. fn` é o nome da função.
Isso não é uma coincidência!
Como você aprenderá na próxima seção, você pode usar o argumento `.names` para fornecer sua própria especificação de cola (*glue*).

### Nomes de colunas

O resultado de `across()` é nomeado de acordo com a especificação fornecida no argumento `.names`.
Poderíamos especificar o nosso próprio se quiséssemos que o nome da função viesse primeiro[^iteration-3]:

[^iteration-3]: Atualmente você não pode alterar a ordem das colunas, mas pode reordená-las após o fato usando `relocate()` ou similar.
...
Portanto, dplyr fornece duas variantes de `across()` chamadas `if_any()` e `if_all()`:
...
Por exemplo, [Jacob Scott](https://twitter.com/_wurli/status/1571836746899283969) usa esta pequena função auxiliar que envolve um monte de funções do pacote lubridate para expandir todas as colunas de data em colunas de ano, mês e dia:
...
Esta é uma técnica útil para conhecer porque às vezes você encontrará um problema que atualmente não é possível resolver com `across()`: quando você tem grupos de colunas com os quais deseja calcular simultaneamente.
...
3.  Ajuste `expande_datas()` para remover automaticamente as colunas de data após elas terem sido expandidas.
    Você precisa abraçar algum argumento?

4.  Explique o que faz cada etapa do pipeline nesta função.
    De qual recurso especial de `where()` estamos aproveitando?
...
Existem três etapas básicas: use `list.files()` para listar todos os arquivos em um diretório, depois use `purrr::map()` para ler cada um deles em uma lista e, em seguida, use `purrr::list_rbind( )` para combiná-los em um único quadro de dados.
...
Para tornar concreto nosso exemplo motivador, este livro contém uma pasta com 12 planilhas Excel contendo dados do pacote gapminder.
Cada arquivo contém dados de um ano para 142 países.
...
Mas colocar cada planilha em sua própria variável tornará difícil trabalhar com elas daqui a alguns passos.
Em vez disso, será mais fácil trabalhar com eles se os colocarmos em um único objeto.
...
Agora que você tem esses *data frames* em uma lista, como retirá-los?
Você pode usar `files[[i]]` para extrair o i<sup>ésimo</sup> elemento:

```{r}
arquivos[[3]]
```

Voltaremos a `[[` com mais detalhes no @sec-subset-one.
...
Agora podemos usar `purrr::list_rbind()` para combinar essa lista de quadros de dados em um único *data frame*:
...
Às vezes, o nome do arquivo são os próprios dados.
Neste exemplo, o nome do arquivo contém o ano, que de outra forma não é registrado nos arquivos individuais.
Para colocar essa coluna no quadro de dados final, precisamos fazer duas coisas:

Primeiro, nomeamos o vetor de caminhos (*paths*).
A maneira mais fácil de fazer isso é com a função `set_names()`, que pode assumir uma função.
...
Em casos mais complicados, pode haver outras variáveis ​​armazenadas no nome do diretório ou talvez o nome do arquivo contenha outros dados.
...
Agora que você fez todo esse trabalho duro para chegar a um quadro de dados bem organizado, é um ótimo momento para salvar seu trabalho:
...
Recomendamos essa abordagem porque ela evita que você se preocupe em acertar o primeiro arquivo antes de passar para o resto.
...
Neste exemplo específico, há outra otimização que você pode fazer, vinculando todos os *data frames* anteriormente.
...
Infelizmente, às vezes não é possível ir de `map()` direto para `list_rbind()` porque os *data frames* são tão heterogêneos que `list_rbind()` falha ou produz um *data frame* que não é muito útil.
...
Então, uma estratégia muito útil é capturar a estrutura dos quadros de dados para que você possa explorá-la usando suas habilidades em ciência de dados.
...
[^iteração-6]: Não vamos explicar como funciona, mas se você olhar a documentação para as funções usadas, você conseguirá decifrar.
...
Mas não nos importamos com a saída de `adiciona_arquivo()`, então em vez de `map()` é um pouco melhor usar `walk()`.
...
`transparencia` é nossa variável de agrupamento e `data` é uma coluna de lista contendo um tibble para cada valor único de `transparencia`:
...
sso é um pouco diferente dos nossos usos anteriores de `map()` porque há dois argumentos que estão mudando, não apenas um.
...
No entanto, você verá loops `for` em códigos por aí, então aprenderá sobre eles no próximo capítulo, onde discutiremos algumas ferramentas do R base importantes.
