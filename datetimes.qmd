...
Você os usa o tempo todo em sua vida normal e eles não parecem causar muita confusão.
...
Este capítulo não ensinará todos os detalhes sobre datas e horários, mas fornecerá uma base sólida de habilidades práticas que ajudarão com desafios comuns de análise de dados.
...
A partir da versão mais recente tidyverse, o lubridate é um componente principal do tidyverse.
Também precisaremos do conjunto de dados voos do pacote dados, para casos práticos.
...
Se você precisar disso, você pode usar o pacote **hms**.
...
Observe que não importa como você especifica o formato da data, ele sempre será exibido da mesma maneira quando você o inserir no R.
...
Isso lhe dá o nome da função lubridate que analisará sua data.
Por exemplo:
...
. Ele não usa o horário de verão, tornando um pouco mais fácil calcular
.
...
Para criar uma data/horario a partir deste tipo de entrada, use a `make_date()` para datas, or `make_datetime()` for data-horário:
...
Vamos fazer a mesma coisa para cada uma das quatro colunas de tempo em `flights`.
...
#|   nos fins de semana. São poucos os dias que se destacam por terem sido surpreendentemente
...
#|   eixo-x, número de voos no eixo-y (0-17), agrupados em 10 minutos
#|   incrementos. É difícil ver muitos padrões devido à alta variabilidade,
...
Esse é o trabalho de `as_datetime()` e `as_date()`:
...
Às vezes, você obterá datas/horários como compensações numéricas (*offsets*) da "Época Unix" (*Unix epoch*), 01/01/1970.
...
A próxima seção verá como a aritmética funciona com data e horário.
...
#|   vôos no eixo-y. Segunda a sexta-feira têm aproximadamente o mesmo número de
...
Curiosamente, se olharmos para o horário de saída *programado*, não vemos um padrão tão forte:
