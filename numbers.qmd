...
Vetores numéricos são as espinha dorsal da ciência de dados e você já os usou muitas vezes anteriormente neste livro.
...
readr fornece duas funções úteis para transformar *strings* em números: `parse_double()` e `parse_number()`.
...
É uma supresa o quanto de ciência de dados você pode fazer somente com um pouco de contagem e aritmética básica, então o pacote dplyr torna a contegam o mais fácil possível com `count()`.
...
Como um exemplo, enquanto o R fornecer todas as funções trigonemétricas que você pode sonhar, não iremos listá-las aqui pois elas são raramente necessárias em ciência de dados.
...
Isto é importante para operações como `voos |> mutate(tempo_voo = tempo_voo / 60)` pois existem 336.776 múmeros do lado esquerdo da `/` mas apenas um no lado direito.
...
Em geral (mas nem sempre), te retorna uma mensage de aviso se o vetor maior não é um múltiplo do menor:
...
O código é exeutado sem erros, mas não retorna o que você espera.
Devido às regras de reciclagem, ele encontra um número par de linhas que saíram em Janeiro e número ímpar de voos que saíram em Fevereiro.
E infelizmente, não há aviso de erro, pois `voos` possui um número ímpar de linhas.
...
Infelizmente isto não ajuda aqui, ou em vários outros casos, pois o cálculo é feito pel função `==` do R base e não pela `filter()`.
...
Aritmética é útil para o conjunto de dados `voos`, pois podemos usá-la para quebrar a variável `saida_programada` em `hora` e `minuto`:
...
#|   de serem cancelados.
#| fig-alt: |
#|   Um gráfico de linha mostrando como a proporção de de voos cancelados muda 
#|   ao longo do dia. A proporção começa baixa, de aproximandamente 0.5% às 06:00 horas 
#|   e então aumenta de forma uniforme ao longo do dia até um pico 
#|   de 4% às 19:00m horas. A proporção de voos cancelados cai então rapidamente
...
`log2()` é fácil interpretar pois a diferença de 1 na escala logaritma corresponde ao dobro na escalada original e a diferença de -1 corresponde à metade; e o `log10()` é de fácil transformação inversa (*back-transform*) (e.x.) 3 é 10\^3 = 1000.
O inverso de `log()` é `exp()`; para calcular o inverso de `log2()` ou `log10()` você precisará usar `2^` ou `10^`.

### Arrendondamento {#sec-rounding}
...
`round(x, digits)` arredonda para o próximo `10^-n` então `digits = 2` irá arredondar para o próximo 0.01 mais próximo.
Esta definição é útil, pois implica que `round(x, -3)` arredondará para o milésimo mais próximo, o que de fato acontece:

```{r}
round(123.456, 2)  # dois digitios
round(123.456, 1)  # um digito
...
`round()` usa o que é conhecido como "arredondamento da metada para o par" ou arredondamento do banqueiro: se um número estiver no meio do caminho entre dois inteiros, ele será arredondado para o inteiro **par**.
...
Você pode, opcionalmente, deinir seus próprios rótulos (`labels`).
...
dplyr oferece `cummean()` para média móvel.
...
Se você precisar de agregadores cumulativos ou rolantes mais complexos, tente o pacote [slider](https://slider.r-lib.org/).
...
2.  Quais funções de trigonometria o R oferece?
    Chute alguns nomes e veja a documentação.
...
3.  Atualmente `horario_saida` e `saida_programada` são convenientes para consultá-las, mas difícil de calculá-las pois elas não são realmente números contínuos.
    Você pode ver o problema básico executando o código abaixo: há um espaço (*gap*) entre cada hora.
...
4.  Arredonde `horario_saida` and `horario_chegada` para os próximos cinco minutos mais próximos.
...
dplyr oferece diversas funções de ranqueamento inspiradas no SQL, mas você deve sempre começar com `dplyr::min_rank()`.
...
`row_number()` também pode ser usada sem nenhu argumento dentro de um verbo dplyr.
Neste caso, irá te retornar o número da linha "atual".
Quando combinado com `%%` ou `%/%` isto pode ser uma ferramenta útil para dividor os dados em grupos de tamanhos parecidos:
...
Você pode usar determinar o número de posições para frente (*lead*) ou para trás (*lag*) usando o segundo argumento `n`.
...
Por exemplo, imagine que você tenha os horários em que alguém visitou um o site:
...
E você calculou o período entre cada ocorrência de eventos, e descobri se há há um intervalo grande o suficiente para se qualificar:
...
Por exemplo, inpirado [nesta pergunta do stackoverflow](https://stackoverflow.com/questions/27482712), imagine que você tenha um *data frame* com vários valores repetidos:
...
2.  Qual avião (`codigo_cauda`) tem o pior histórico de pontualidade has the worst on-time record?

3.  A que horas você deve voar se quiser evitar atrasos o máximo possível?
...
6.  Os atrasos são normalmente correlacionados temporalmente: mesmo depois de o problema que causou o atraso inicial ter sido resolvido, os voos posteriores são atrasados ​​para permitir a saida dos voos anteriores.
...
7.  Veja cada destino.
    ...
    Calcule o tempo de voo de um voo em relação ao voo mais curto para esse destino.
    Quais voos tiveram mais atrasos no ar?

8.  Encontre todos os destinos operados por pelo menos duas companhias aéreas.
    Use esses destinos para chegar a um ranqueamento relativa das transportadoras com base no seu desempenho para o mesmo destino.
...
Dependendo da forma da distribuição da variável de interesse, média ou mediana podem ser uma melhor medidade de posição.
...
A mediana de atraso é sempre menor que a média de atraso, pois em alguns casos, os voos saem com árias horas de atraso, mas nunca saem várias horas antecipadamente.

```{r}
#| label: fig-mean-vs-median
#| fig-cap: |
#|   Um gráfico de dispoersão mostrando a diferença entre sumarizar atraso das saídas diárias
#|   com mediana ao invés da média.
#| fig-alt: |
#|   Todos os pontos estão sob a linha de 45°, signifcando que o atraso da mediana é
#|   sempre que o atraso médio. A maioria dos pontos estão agrupados em um
#|   região desna da média [0, 20] e mediana [0, 5]. Conforme o atraso  médio aumenta
...
Você também pode estar se perguntando sobre a **moda**, ou valor mais comum.
...
Se o dados for discreto, podem existir muitos valores comuns, e se o data for contínuo, podem não existir valores comuns pois cada valor é ligeiramente diferente um do outro.
...
E se você estiver interessados em posições diferentes do centro?
...
Para os dados de `voos`, você pode querer olhar no quantil 95% dos voos em atraso ao invés no valor máximo, pois isto ignora os voos com maior atraso os quais podem ser muito extremos.
...
Não iremos explifcar o `sd()` aqui, pois você já deve ser algo familiar a você, mas `IQR()` pode ser algo novo, --- ele é `quantile(x, 0.75) - quantile(x, 0.25)` e retornar um intervalo ue contém a metade (50%) dos dados.

Podemos usar isso para revelar uma pequena estranheza nos dados dos `vôos`.
...
#|   difícil obter detalhes. (Direita) Ampliando atrasos de menos de duas
...
Há um último tipo de sumarização útil para vetores numéricos, que também funciona para outros tipos de valores: extratir um valor em uma posição específica: `first(x)`, `last(x)` e `nth(x, n)`.
...
Extrair valores em determninada posição é complementar a filtrar em ranqueamentos.
Filtrar retorna todas as variáveis, com cada observações em uma linha separada:
...
2.  Quais destinos apresentam a maior variação na velocidade do ar?
...
Você já está familiarizado com muitas ferramentas para trabalhar com números e, depois de ler este capítulo, agora sabe como usá-las no R.
...
Por fim, você trabalhou com várias sumarizações numéricos e discutiu alguns dos desafios estatísticos que deveria considerar.
