...
#|   arrumar, transformar, visualizar, modelar e comunicar, ou seja, tudo)
#|   destacada em azul.
...
Escrever códigos claros é importante para que outros (como você do futuro) possam entender o porquê de você ter abordado uma análise da forma como fez.
...
1.  Copiar e colar é uma ferramenta poderosa, mas você deve evitar usá-la mais que duas vezes.
    Código repetitivo é perigoso, pois ele pode facilmente te levar a erros e inconsistências.
...
3.  Quanto mais você ler códigos escritos por outras pessoas, mais você verá códigos que não utilizam tidyverse.
    No @sec-base-r, você irá aprender algumas das funções mais importantes do R base que você vê por aí.

O objetivo desses capítulos é te ensinar o mínimo sobre programação que você precisa para ciência de dados.
Quando você dominar esse material, recomendamos fortemente que você continue a investir em suas habilidades de programação.
Nós escrevemos dois livros que você pode achar útil.
[*Hands on Programming with R*](https://rstudio-education.github.io/hopr/), de Garrett Grolemund, é uma introdução a R como linguagem de programação e um ótimo ponto de partida se R é sua primeira linguagem de programação.
[*Advanced R*](https://adv-r.hadley.nz/) de Hadley Wickham entra em detalhes na linguagem de programação R; é um ótimo ponto de partida se você já tem experiência em programação e ótimo próximo passo se você já tiver internalizado as ideias desses capítulos.
