...
Começaremos discutindo algumas ferramentas gerais para trabalhar com valores faltantes (*missing values*) registrados como `NA`s.
Em seguida, exploraremos a ideia de valores faltantes implícitos, ou seja, valores que estão simplesmente ausentes dos seus dados, e mostraremos algumas ferramentas que você pode usar para torná-los explícitos.
...
Funciona como a `select()`, pegando um conjunto de colunas:
...
Antes de continuarmos, há um tipo especial de valor ausente que você encontrará de tempos em tempos: um `NaN` (pronuncia-se "nan") ou em inglês, **n**ot **a** **n**umber .
Não é tão importante saber porque geralmente se comporta como `NA:
...
Uma maneira de pensar sobre a diferença é como este diálogo meio Zen:

> Um valor ausente explícito é a presença de uma ausência.\
>
> Um valor ausente implícito é a ausência de presença.

Às vezes, você deseja tornar explícitas os valores ausentes implícitos para ter algo físico com o qual trabalhar.
Em outros casos, valores fatantes explícitos são impostos pela estrutura dos dados e você deseja se livrar deles.
As seções a seguir discutem algumas ferramentas para alternar entre ausências implícitas e explícitas.

### Pivotagem

Você já viu uma ferramenta que pode tornar explícitas as ausências implícitas e vice-versa: pivotagem (*pivoting*).
Ampliar os dados pode tornar explícitos os valores ausentes implícitos porque cada combinação de linhas e novas colunas deve ter algum valor.
Por exemplo, se pivotarmos `ações` para colocar o `trimestre` nas colunas, ambos os valores faltantes se tornarão explícitos:
...
Veja os exemplos no @sec-tidy-data para mais detalhes.
...
Nesse caso, você pode fazer manualmente o que `complete()` faz por você: criar um quadro de dados que contenha todas as linhas que deveriam existir (usando qualquer combinação de técnicas necessárias) e, em seguida, combiná-lo com seu conjunto de dados original com `dplyr ::full_join()`.

### Uniões 

Isso nos leva a outra forma importante de revelar observações implicitamente ausentes: uniões (*joins*).
Você aprenderá mais sobre uniçoes no @sec-joins, mas gostaríamos de mencioná-las rapidamente aqui, já que muitas vezes você só pode saber que valores estão faltando em um conjunto de dados quando você o compara com outro.
...
Um último tipo de ausência é a de grupo vazio (*empty group*), um grupo que não contém nenhuma observação, que pode surgir ao trabalhar com fatores (*factors*).
...
#|   - O mesmo gráfico de barras do último gráfico, mas agora com dois valores ativados
#|     o eixo-x, "sim" e "não". Não há barra para a categoria “sim”.
...
Valores faltantes (*missing values*) são estranhos!
...
No próximo capítulo, abordamos o capítulo final desta parte do livro: uniões (*joins*).
