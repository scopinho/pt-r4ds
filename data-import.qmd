...
Trabalhar com dados fornecidos por pacotes R é uma ótima maneira de aprender ferramentas de ciência de dados, mas você irá querer aplicar o que aprendeu aos seus próprios dados em algum momento.
Neste capítulo, você aprenderá o básico de como ler arquivos de dados no R.
...
Você então aprenderá sobre como ler dados de múltiplos arquivos de uma vez e exportar dados do R para um arquivo.
Finalmente, você aprenderá a criar manualmente data frames no R.

### Pré-requisitos

Neste capítulo, você irá aprender como importar arquivos planos no R com o pacote **readr**, que faz parte do tidyverse.
...
## Importando dados de um arquivo
...
Podemos importar esse arquivo para o R usando `read_csv()`.
O primeiro argumento é o mais importante: o caminho para o arquivo.
Você pode pensar no caminho como um endereço do arquivo: o nome do arquivo é `estudantes.csv` e ele mora na pasta `data`.
...
<!--
Ao revisor:
Os links abaixo só funcionarão após o PR ser mesclado
-->
...
Na coluna `camida.favorita`, há uma série de itens de comida, e então a *string* de caracteres `N/A`, que deveria ter sido um `NA` real que o R reconheceria como "não disponível" (*not available*).
...
Outra tarefa comum após a importação dos dados é a correção dos tipos de variáveis.
...
<!--
Ao revisor:
No parágrafo a seguir, estou traduzindo `id` para `student_id`, pois não há uma coluna chamada `id`.
-->

Antes de analisar esses dados, você provavelmente vai querer corrigir as colunas `idade` e `id`.
Atualmente, `idade` é uma variável de caractere porque uma das observações é digitada como `cinco` em vez de um `5` numérico.
...
Você pode usar `skip = n` para pular as primeiras `n` linhas ou usar `comment = "#"` para descartar todas as linhas que começam com `#`, por exemplo:
...
Você pode usar `col_names = FALSE` para dizer ao `read_csv()` para não tratar a primeira linha como cabeçalhos e, em vez disso, rotulá-los sequencialmente de `X1` a `Xn`:
...
Alternativamente, você pode passar a `col_names` um vetor de caracteres que será usado como os nomes das colunas:
...
-   `read_delim()` lê arquivos com qualquer delimitador, tentando adivinhar automaticamente o delimitador se você não o especificar.
...
6.  Pratique se referir a nomes não-sintáticos no seguinte data frame por:

    a.  Extrair a variável chamada `1`.
    b.  Plotar um gráfico de dispersão de `1` vs. `2`.
    c.  Criar uma nova coluna chamada `3`, que é `2` dividido por `1`.
    d.  Renomear as colunas para `um`, `dois` e `três`.
...
Um arquivo CSV não contém informações sobre o tipo de cada variável (ou seja, se é um lógico, número, *string*, etc.), então o readr tentará adivinhar o tipo.
...
readr usa uma heurística para descobrir os tipos de coluna.
...
readr fornece um total de nove tipos de coluna para você usar:

-   `col_logical()` e `col_double()` leem lógicos e números reais. Eles são relativamente raramente necessários (exceto como acima), já que o readr geralmente os adivinhará para você.
-   `col_integer()` lê inteiros. Raramente distinguimos inteiros e duplos neste livro porque eles são funcionalmente equivalentes, mas ler inteiros explicitamente pode ocasionalmente ser útil porque ocupam metade da memória dos reais.
-   `col_character()` lê *strings*. Isso pode ser útil para especificar explicitamente quando você tem uma coluna que é um identificador numérico, ou seja, longas séries de dígitos que identificam um objeto, mas não faz sentido aplicar operações matemáticas a ele. Exemplos incluem números de telefone, números de seguro social, números de cartão de crédito, etc.
...
Outra função auxiliar é `cols_only()`, que lerá apenas as colunas que você especificar:
...
read_csv(arquivos_vendas, id = "file")
```
...
readr também vem com duas funções úteis para escrever dados de volta para o disco: `write_csv()` e `write_tsv()`.
Os argumentos mais importantes para essas funções são `x` (o data frame a ser salvo) e `file` (o local para salvá-lo).
Você também pode especificar como os valores ausentes são escritos com `na`, e se você deseja acrescentar, via o argumento `append`, a um arquivo existente.
...
Agora, vamos ler esse arquivo CSV de volta.
...
1.  `write_rds()` e `read_rds()` são *wrappers* uniformes[^data-import-4] em torno das funções base `readRDS()` e `saveRDS()`.
    Eles armazenam dados no formato binário personalizado do R chamado RDS.
    Isso significa que quando você recarrega o objeto, você está carregando o *mesmo* objeto R que você armazenou.
...
Inserir os dados por coluna pode dificultar a visualização de como as linhas estão relacionadas, então uma alternativa é `tribble()`, abreviação de _**tr**ansposed t**ibble**_, que permite organizar seus dados linha por linha.
...
Neste capítulo, você aprendeu a como carregar arquivos CSV com `read_csv()` e a como inserir seus próprios dados com `tibble()` e `tribble()`.
Você aprendeu como arquivos CSV funcionam, alguns dos problemas que você pode encontrar e como superá-los.
Voltaremos à importação de dados algumas vezes neste livro: @sec-import-spreadsheets do Excel e Google Planilhas, @sec-import-databases mostrará como carregar dados de bancos de dados, @sec-arrow de arquivos parquet, @sec-rectangling de JSON e @sec-scraping de sites.

Estamos quase no final desta seção do livro, mas há um último tópico importante a ser coberto: como obter ajuda.
Então, no próximo capítulo, você aprenderá alguns bons lugares para procurar ajuda, como criar um reprex[^data-import-5] para maximizar suas chances de obter boa ajuda e alguns conselhos gerais sobre como acompanhar o mundo do R.

[^data-import-1]: No Brasil, é mais comum o tipo CSV2, que utiliza ponto-e-vírgula como separador e é, portanto, compatível com o uso da vírgula como separador decimal. (N. dos T.)
