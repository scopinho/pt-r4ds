...
Os princípios do formato _tidy_ parecem ser tão óbvios que você deve estar se perguntando se algum dia vai encontrar algum cojunto de dados que não esteja nesse formato.
...
Isso significa que a maioria das análises reais vai demandar pelo menos alguma adaptação para o formato _tidy_.
...
Em seguida, você irá *pivotar** seus dados para o formato _tidy_, com as variáveis nas colunas e as observações nas linhas. 
...
Para transformar esses dados em _tidy_, vamos usar `pivot_longer()`:
...
Esses dados agora estão no formato _tidy_, mas nós ainda podemos facilitar cálculos futuros ao converter os valores da coluna `week` de caracteres para números utilizando `mutate()` e `readr::parse_number()`.
...
Agora que você já viu como usar a pivotagem para mudar a forma de nossos dados, vamos tomar um tempinho para obter alguma intuição sobre o que a pivotagem faz com os dados.
