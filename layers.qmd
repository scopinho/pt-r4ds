...
No @sec-data-visualization, você aprendeu muito mais do que apenas como criar gráficos de dispersão, gráficos de barras e boxplots.
...
Vamos começar explorando mais profundamente o mapeamento estético, os objetos geométricos e as facetas.
Então, você irá aprender sobre as transformações estatísticas que acontecem nos bastidores quando o ggplot2 cria um gráfico.
...
Você também irá aprender sobre ajustes de posição, os quais modificam como os objetos geométricos são exibidos nos seus gráficos.
...
De forma semelhante, podemos também mapear a variável `classe` nos atributos estéticos `size` (tamanho) ou `alpha`, as quais controlarão o tamanho e a transparência dos pontos, respectivamente.
...
Ele seleciona uma escala adequada para ser utilizada com esse atributo e constroi uma legenda que explica a relação entre as categorias e os valores.
...
Uma linha escreve todos os pontos pertencentes à categoria `4`, outra linha descreve os pontos pertencentes à categoria `d`, e uma terceira linha descreve todos os pontos pertencentes à categoria `t`.
...
Muitas geometrias, como o `geom_smooth()`, usam um único objeto geométrico para representar várias linhas de dados.
Para essas geometrias, você pode mapear o atributo estético `group` (grupo) a uma variável categórica para desenhar vários objetos geométricos.
O ggplot2 irá desenhar um objeto geométrico separado para cada valor único da variável de agrupamento.
...
#|   entre a eficiência de combustível na estrada e o tamanho do motor dos carros
...
No gráfico a seguir, não apenas nós utilizamos uma nova geometria (`geom_density_ridges()`), mas também mapeamos a mesma variável a múltiplos atributos estéticos (`tracao` foi mapeada em `y`, `fill` e `color`), além de definirmos um atributo estético (`alpha = 0.5`) para tornar as curvas de densidade transparentes.
...
4.  Observe o primeiro gráfico criado por faceta abaixo:
    ...
    Quais as vantagens de usar facetas ao invés do atributo estético `color`?
...
#|   Finalmente, essa variável calculada (a contagem) é mapeada para a estética
#|   do gráfico.
#| fig-alt: |
...
2.  Você pode querer substituir o mapeamento padrão de variáveis transformadas para estética.
...
Você pode colorir um gráfico de barras usando a estética `color` ou, de maneira mais útil, a estética `fill`:
...
Observe o que acontece se você mapear a estética de preenchimento (`fill`) para outra variável, como `classe`: as barras são empilhadas automaticamente.
...
Você poderia então usar as propriedades estéticas das geometrias para representar variáveis presentes nesses dados.
...
Em seguida, você selecionaria um sistema de coordenadas para posicionar os objetos geométricos, usando a localização dos objetos (que é em si uma propriedade estética) para exibir os valores das variáveis x e y.
