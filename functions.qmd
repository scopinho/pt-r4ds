...
Esses exemplos não seriam possíveis sem a ajuda do pessoal do Twitter, e encorajamos você seguir os links nos comentários para ver as inspirações originais.
...
Você pode advinhar que isso redimensiona cada coluna para ter um intervalo de 0 a 1.
...
Se pegarmos o código acima e extraí-lo da função `mutate()`, será um pouco mais fácil ver o padrão porque cada repetição agora é uma linha:
...
Para deixar isso um pouco mais claro, podemos substituir o bit que varia com `█`:
...
(No @sec-iteration, ocê aprenderá como usar `across()` para reduzir ainda mais a duplicação, então tudo que você precisará é `df |> mutate(across(a:d, rescala01))`).

### Melhorando nossa função

Você pode notar que a função `rescala01()` faz algum trabalho desnecessário --- em vez de calcular `min()` duas vezes e `max()` uma vez que poderíamos calcular o mínimo e o máximo em uma única etapa com `range( )`:
...
Essas mudanças ilustram um benefício importante das funções: como movemos o código repetido para uma função, só precisamos fazer a mudança em um só lugar.
...
Outra família importante de funções vetoriais são as funções de resumo, funções que retornam um único valor para uso em `summarize()`.
...
Para permitir que você escreva uma função que use verbos dplyr, primeiro apresentaremos o desafio da indireção (*indirection challenge*) e como você pode superá-lo usando `{{ }}`.
...
Abraçar (*embracing*) uma variável significa envolvê-la entre colchetes para que (por exemplo) `var` se torne `{{ var }}`.
...
Além disso, uma vez que os argumentos da `summarize()` são usam mascamento de dados (*data-masking*), isso significa que o argumento `var` para a função `sumario6()` também é usa mascaramento de dados (*data-masking*).
...
Por exemplo, imagine que você deseja escrever um `conta_faltantes()` que conte o número de observações faltantes nas linhas.
...
1.  Usando os conjuntos de dados voos e clima do pacote dados, escreva uma função que:
    ...
    2.  Conta o número de voos cancelados e o número de voos atrasados ​​mais de uma hora.
...
2.  Para cada uma das funções a seguir, liste todos os argumentos que usam avaliação organizada (*tidy-evaluation) e descreva se eles usam mascaramento de dados (*data-masking*) ou seleção organizada (tidy-selection): `distinct()`, `count()`, `group_by()`, `rename_with()`, `slice_min()`, `slice_amostra()`.
...
Alguns das funções auxiliares mais úteis combinam uma pitada de manipulação de dados com ggplot2.
...
```{r}
#| fig-alt: |
#|   Gráfico de barras de transparência de diamantes, onde a clareza está no eixo y e conta 
...
Lembre-se da função de histograma que mostramos anteriormente?
...
Não seria bom se pudéssemos rotular a saída com a variável e a largura do compartimento que foi usada?
...
Além disso, `funcao()` deve sempre ser seguida por colchetes (`{}`), e o conteúdo deve ser recuado por dois espaços adicionais.
