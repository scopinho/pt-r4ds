...
Neste capítulo, exploraremos os cinco conjuntos de dados relacionados do pacote nycflights13[^joins-1] usando as funções de união do pacote dplyr.

[^joins-1]: **Nota de tradução**: Disponíveis em português no pacotes [dados](https://cienciadedatos.github.io/dados/).
...
Quando são necessárias mais de uma variável, a chave é denominada de chave primária composta (*compound primary key*).
Por exemplo, no pacote dados:
...
Para esses exemplos, facilitaremos a visualização do que está acontecendo criando um conjunto de dados mais restrito com apenas seis variáveis[^joins-2]:

[^joins-2]: Lembre-se que no RStudio você também pode usar `View()` para evitar este problema
...
É especial porque a saída sempre terá as mesmas linhas de `x`, o *data frame* ao qual você está unindo[^joins-3].
...
[^joins-3]: That's not 100% true, but you'll get a warning whenever it isn't.
...
```{r}
voos2 |> 
  anti_join(aeroportos, join_by(destino == codigo_aeroporto)) |> 
  distinct(destino)
```
...
A @fig-join-setup2 apresenta a base para nossa representação visual Ele mostra todas as correspondências potenciais entre `x` e `y` como a interseção entre as linhas desenhadas de cada linha de `x` e cada linha de `y`..
...
União não-equivalente (*non-equi join*) não é um termo particularmente útil porque apenas informa o que a união não é, e não o que é.
O dplyr ajuda a identificar quatro tipos particularmente úteis de uniões não-equivalentes:
