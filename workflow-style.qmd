...
Se a função não tiver argumentos nomeados (como `select()` ou `filter()`), mantenha tudo em uma linha, caso caiba.
Caso não caiba, você deve colocar cada argumento em sua própria linha.
...
Dê nomes informativos sempre que puder.
Por exemplo, quando você alterar a estrutura dos dados em seu nível básico, como após pivotar ou sumarizar.
