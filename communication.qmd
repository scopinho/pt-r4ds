...
No @sec-exploratory-data-análise, você aprendeu como usar gráficos como ferramentas para *exploração*.
...
No decorrer da maioria das análises, você produzirá dezenas ou centenas de gráficos, a maioria dos quais será imediatamente descartado.
...
Para ajudar outros a construir rapidamente um bom modelo mental dos dados, você precisará investir um esforço considerável para tornar seus gráficos o mais autoexplicativos possível.
...
Gostamos particularmente de [The Truthful Art](https://www.amazon.com/gp/product/0321934075/), de Albert Cairo.
...
Também usaremos um pouco de dplyr para manipulação de dados, **scales** para substituir quebras, rótulos, transformações e paletas padrão, e alguns pacotes de extensão ggplot2, incluindo **ggrepel** ([https://ggrepel. slowkow.com](https://ggrepel.slowkow.com/)) de Kamil Slowikowski e **patchwork** ([https://patchwork.data-imaginist.com](https://patchwork.data-imaginist .com/)) de Thomas Lin Pedersen.
Usaremos também o pcaote *dados* e seu conjuntos de dados *milhas*, *predisentes_eua* e *diamante*.
...
#| message: false
#| fig-alt: |
#|   Gráfico de dispersão da eficiência de combustível nas rodovias versus tamanho do motor dos carros, onde
...
#|   o tamanho do motor dos carros é sobreposto. O eixo x é rotulado como "Cilindrada  
#|   do Motor (L)" e o eixo y é rotulado como "Economia de combustível na rodovia (mpg)". 
#|   A legenda é rotulada como "Tipo de veículo". O gráfico é intitulado "Eficiência de combustível 
#|   geralmente diminui com o tamanho do motor". O subtítulo é "Dois lugares
#|   (carros esportivos) são uma exceção devido ao seu peso leve" e o 
#|    a legenda é "Dados de fueleconomy.gov".
...
Se você precisar adicionar mais texto, existem dois outros argumentos de rótulos úteis: `subtitle` (subtítulo) adiciona detalhes adicionais em uma fonte menor abaixo do título e `caption` (legenda) adiciona texto no canto inferior direito do gráfico, geralmente usado para descrever a fonte do dados.
...
1. Crie um gráfico nos dados de economia de combustível com rótulos personalizados de `title`, `subtitle`, `caption`, `x`, `y` e `color`.
...
2. Recrie o gráfico a seguir usando os dados de economia de combustível.
    ...
    #| echo: false
    #| fig-alt: |
    #|   Gráfico de dispersão da eficiência de combustível em rodovias versus cidades. Formas e
    #|   as cores dos pontos são determinadas pelo tipo de trem de força.
...
3.  Pegue um gráfico exploratório que você criou no último mês e adicione títulos informativos para facilitar a compreensão de outras pessoas.
...
Existem duas fontes possíveis de rótulos.
Primeiro, você pode ter um arquivo que fornece rótulos.
No gráfico a seguir, retiramos os carros com maior tamanho de motor em cada tipo de direção e salvamos suas informações como um novo quadro de dados chamado `label_info`.
...
(`theme(legend.position = "none"`) desativa todas as legendas --- falaremos sobre isso mais em breve.

```{r}
#| fig-alt: |
#|   Gráfico de dispersão da eficiência na rodovia versus tamanho do motor onde os pontos são coloridos
#|   por tipo de unidade. Curvas suaves para cada tipo de unidade são sobrepostas. 
...
Este pacote útil ajustará automaticamente os rótulos para que não se sobreponham:

```{r}
#| fig-alt: |
#|   Gráfico de dispersão da eficiência de combustível nas rodovias versus cilindradas, onde 
#|   os pontos são coloridos de acordo com a classe de tração do carro. Alguns pontos são rotulados 
#|   com o nome do carro. As etiquetas são caixas com fundo branco e transparente 
#|   e posicionado para não se sobrepor.
...
```{r}
#| fig-alt: |
#|   Gráfico de dispersão da eficiência de combustível nas rodovias versus as cilindradas dos carros. Pontos
#|   onde a quilometragem da rodovia está acima de 40 e também acima de 20 com tamanho do motor 
...
Lembre-se, além de `geom_text()` e `geom_label()`, você tem muitos outras geometrias (*geoms*) no ggplot2 disponíveis para ajudar a anotar seu gráfico.
Algumas ideias:

- Use `geom_hline()` e `geom_vline()` para adicionar linhas de referência.
    Freqüentemente, os tornamos grossos (`linewidth = 2`) e brancos (`color = white`) e os desenhamos abaixo da camada de dados primária.
...
- Use `geom_rect()` para desenhar um retângulo ao redor dos pontos de interesse.
    ...
    Como alternativa, consulte o [pacote ggforce](https://ggforce.data-imaginist.com/index.html), especificamente [`geom_mark_hull()`](https://ggforce.data-imaginist.com/reference/ geom_mark_hull.html), que permite anotar subconjuntos de pontos com cascas.
...
```{r}
tendencia_texto <- "Motores maiores tendem a ter menor economia de combustível." |>
  str_wrap(width = 30)
tendencia_texto
```
...
```{r}
#| fig-alt: |
#|   Gráfico de dispersão da eficiência de combustível nas rodovias versus cilindradas dos carros. Uma seta
#|   vermelha apontando para baixo segue a tendência dos pontos e da anotação 
#|   colocada ao lado da seta diz "Motores maiores tendem a ter menor 
#|   economia de combustível". A seta e o texto da anotação são vermelhos.
...
4. Quais argumentos para `geom_label()` controlam a aparência da caixa de fundo?
...
Existem muitas escalas não padrão que você aprenderá a seguir.
...
Os rótulos (*labels*) controlam o o texto associado a cada marcador/chave.
...
```{r}
#| fig-alt: |
#|   Gráfico de dispersão da eficiência de combustível nas rodovias versus tamanho do motor dos carros, colorido
#|   por tração. Os eixos x e y não possuem rótulos nas marcações dos eixos.
...
#| fig-width: 4
#| fig-alt: |
#|   Dois gráficos boxplot lado a lado de preço versus corte de diamantes. Os valores discrepantes 
...
```{r}
#| fig-alt: |
#|   Gráficos de barras segmentadas de corte, preenchidos com níveis de clareza. No eixo y 
...
```{r}
#| fig-alt: |
#|   Gráfico linear do número de identificação de presidentes em relação ao ano em que iniciaram seu mandato
...
```{r}
#| fig-alt: |
#|   Gráfico de dispersão da eficiência de combustível nas rodovias versus cilindrada dos carros
#|   onde os pontos são coloridos com base na classe do carro. Sobreposto no gráfico está uma 
#|   curva suave. A legenda está na parte inferior e as classes estão listadas 
...
Por exemplo, é mais fácil ver a relação precisa entre `quilate` e `preço` se os transformarmos em log:
...
#| fig-width: 3
#| fig-alt: |
#|   Dois gráficos de preço versus quilate de diamantes. Dados agrupados e a cor dos 
#|   retângulos que representam cada grupo com base no número de pontos que
#|   cair naquele grupo. No gráfico à direita, valores de preço e quilates
...
```{r}
#| fig-alt: |
#|   Gráfico de preço versus quilate de diamantes. Dados agrupados e a cor dos 
#|   retângulos que representam cada grupo com base no número de pontos que
#|   cair naquele grupo. Os rótulos dos eixos estão na escala de dados original.
...
#| fig-width: 3
#| fig-alt: |
#|   Dois gráficos de dispersão de quilometragem da rodovia versus cilindrada onde os pontos são 
#|   coloridos por tipo de tração. O gráfico à esquerda usa o padrão de
#|   paleta de cores ggplot2 e o gráfico à direita usa uma cor diferente
#|   de paleta.
...
Isso também ajudará a garantir que seu enredo seja interpretável em preto e branco.

```{r}
#| fig-alt: |
#|   Dois gráficos de dispersão de consumo em rodovias versus cilindrada dos carros, onde ambas as cores 
...
#| fig-asp: 0.75
#| fig-alt: |
#|   Três gráficos hexadecimais onde a cor dos hexadecimais mostra o número de observações 
#|   que caem naquele grupo hexagonal. O primeiro gráfico usa o padrão contínuo
#|   de escala do ggplot2. O segundo gráfico utiliza o viridis, escala contínua, e o
#|   o terceiro gráfico usa a escala viridis agrupada.
...
#| message: false
#| fig-alt: |
#|   À esquerda, gráfico de dispersão da consumo rodovia versus cilindrada 
...
#|   são mostradas com cilindrada variando apenas de 5 a 6 e o consumo da rodovia 
...
#| warning: false
#| fig-alt: |
#|   À esquerda, gráfico de dispersão de consumo em rodovias versus cilindrada, com
#|   a cilindrada de 5 a 6 e o consumo em rodoviárias variando de
...
#| fig-width: 4
#| fig-alt: |
#|   À esquerda, um gráfico de dispersão de consumo em rodovias versus cilindradas de SUVs.
...
#|   os carros têm tração 4x4 e os demais têm tração traseira, enquanto 
...
#|   entre consumo e cilindrada nas rodovias enquanto que no gráfico de carros compactos 
#|   o relacionamento é muito mais plano.
...
#| fig-width: 4
#| fig-alt: |
#|   À esquerda, um gráfico de dispersão da consumo nas rodovias versus ciliondrada de SUVs.
#|   À direita, um gráfico de dispersão das mesmas variáveis ​​para carros compactos.
#|   Os pontos são coloridos por tipo de tração. Ambos os gráficos são plotados 
#|   na mesma escala para consumo rodoviário, cilindrada e tipo de tração,
...
#|   não há carros compactos com tração traseira. Como as escalas x e y são iguais, 
#|   e vão muito além do mínimo e máximo do consumo na rodovia e cilindradas, 
...
3.  Altere a exibição dos mandatos presidenciais do EUA:
    ...
    d.  Adicionando rótulos de plotagem informativos.
    e.  Fazendo pausas a cada 4 anos (isto é mais complicado do que parece!).
...
#| message: false
#| fig-alt: |
#|   Gráfico de dispersão do consumo em rodovias versus cilindrada dos carros, colorido por classe 
...
Você também pode criar seus próprios temas, se estiver tentando combinar com um estilo corporativo ou de jornal específico.

[^comunicação-2]: Muitas pessoas se perguntam por que o tema padrão tem um fundo cinza.
...
ggplot(milhas, aes(x = cilindrada, y = rodovia, color = tracao)) +
  geom_point() +
  labs(
    title = "Motores maiores tendem a ter menor economia de combustível",
...
1.  Escolha um tema oferecido pelo pacote ggthemes e aplique-o na último gráfico que você fez.
...
O pacote patchwork permite combinar plotagens separadas no mesmo gráfico.
Carregamos este pacote no início do capítulo.

Para colocar dois gráficos próximos um do outro, você pode simplesmente adicioná-los.
...
#| fig-asp: 0.5
#| fig-alt: |
#|   Dois gráficos (um gráfico de dispersão de consumo em rodovias versus cilindrada e um 
#|   boxplots lado a lado de consumo em rodovias versus tração) colocados lado 
#|   a lado.
...
Você também pode criar layouts de plotagem complexos com patchwork.
...
#|   gráfico de dispersão de consumo em rodovias versus cilindrada, o terceiro gráfico é um
#|   gráfico de dispersão da consumo em rodovias versus consumo na cidade, e o terceiro gráfico é
#|   boxplots de consumo em rodovias versus tração) colocado abaixo 
#|   dos demais
...
Isso ocorre porque estamos modificando o tema do gráfico de retalhos em oposição aos ggplots individuais.
A legenda é colocada no topo, dentro de `guide_area()`.
Por fim, também personalizamos as alturas dos vários componentes do nosso patchwork - o guia tem altura 1, os gráficos de caixa 3, os gráficos de densidade 2 e o gráfico de dispersão facetado 4.
Patchwork divide a área que você reservou para seu lote usando esta escala e posiciona os componentes de acordo.
...
#|   Gráficos três e quatro estão abaixo deles. E o quinto gráfico se estende abaixo deles. 
#|   O gráfico patchwork é entitulado "Consumo em cidade e rodovia com diferentes 
...
#|   E o quinto gráfico é um gráfico de dispersão facetado. Cada um desses gráficos mostra
...
(guide_area() / (p1 + p2) / (p3 + p4) / p5) +
  plot_annotation(
    title = "Consumo em cidade e rodovia com diferentes trações",
...
Se você quiser saber mais sobre como combinar e fazer o layout de vários gráficos com patchwork, recomendamos consultar os guias no site do pacote: <https://patchwork.data-imaginist.com>.
...
2.  Usando os três gráficos do exercício anterior, recrie a seguinte gráfico *patchwork*.
    ...
    #| echo: false
    #| fig-alt: |
    #|   Três gráficos: O gráfico 1 é um gráfico de dispersão de consumo nas rodovias versus cilindrada. 
    #|   O gráfico 2 é um gráfico boxplot do consumo em rodovias versus tração. 
    #|   O gráfico 3 consiste em gráficos boxplot de consumo na cidade versus tração. 
    #|   O gráfico 1 está na primeira linha. Os gráficos 2 e 3 estão na próxima linha, cada um vai até 
...
Neste capítulo você aprendeu como adicionar rótulos de plotagem, como título, subtítulo, legenda, bem como modificar rótulos de eixo padrão, usar anotações para adicionar texto informativo ao seu gráfico ou para destacar pontos de dados específicos, personalizar as escalas dos eixos e alterar o tema do seu enredo.
Você também aprendeu como combinar vários gráficos em um único gráfico usando layouts de gráficos simples e complexos.
